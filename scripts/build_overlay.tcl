...
# Generated network sources
set net_sv_files [glob -nocomplain data/sv/model1/*.sv]

# Add overlay Verilog
add_files $v_files

# Add overlay SystemVerilog
if {[llength $sv_files] > 0} {
    add_files $sv_files
}

# Add network SystemVerilog (top.sv, layers, etc.)
if {[llength $net_sv_files] > 0} {
    add_files $net_sv_files
}

# Ensure Vivado treats them as SystemVerilog
...
# Determine model name from network source path
# Example: data/sv/model1 -> model1
set net_dirs [glob -nocomplain data/sv/*]

if {[llength $net_dirs] == 0} {
    error "Could not find data/sv/<model> directory"
}

set model_dir [lindex $net_dirs 0]
